...
class ConfigLoader:
    ...
    def _load_env_config(self) -> Dict[str, Any]:
        ...
        mysql_env = {}
        if os.getenv("TIKTOK_RES_MYSQL_ENABLED") is not None:
            mysql_env["enabled"] = os.getenv("TIKTOK_RES_MYSQL_ENABLED", "").strip().lower() in {
                "1",
                "true",
                "yes",
                "on",
            }
        if os.getenv("TIKTOK_RES_MYSQL_HOST"):
            mysql_env["host"] = os.getenv("TIKTOK_RES_MYSQL_HOST")
        if os.getenv("TIKTOK_RES_MYSQL_PORT"):
            try:
                mysql_env["port"] = int(os.getenv("TIKTOK_RES_MYSQL_PORT"))
            except (TypeError, ValueError):
                logger.warning(
                    "Invalid TIKTOK_RES_MYSQL_PORT value: %s, ignoring",
                    os.getenv("TIKTOK_RES_MYSQL_PORT"),
                )
        if os.getenv("TIKTOK_RES_MYSQL_USER"):
            mysql_env["user"] = os.getenv("TIKTOK_RES_MYSQL_USER")
        if os.getenv("TIKTOK_RES_MYSQL_PASSWORD"):
            mysql_env["password"] = os.getenv("TIKTOK_RES_MYSQL_PASSWORD")
        if os.getenv("TIKTOK_RES_MYSQL_DATABASE"):
            mysql_env["database"] = os.getenv("TIKTOK_RES_MYSQL_DATABASE")
        if mysql_env:
            env_config["mysql"] = mysql_env
        return env_config
