...
### Shared Logic
- This app extends shared Python backend logic from `../content-assets-shared`.
- When fixing shared logic in `auth/`, `cli/`, `config/`, `control/`, `core/`, `storage/`, `tools/`, `utils/`, or shared tests, prefer changing `content-assets-shared` first.
- Keep `content-assets-console` focused on the local console, platform agent, product matching, and app-specific configuration.
