...
class _FakeRedirectResp:
    def __init__(self, status: int, final_url: str, history=None):
        self.status = status
        self.url = final_url
...
class _FakeSession:
    ...
    def get(self, url, allow_redirects=True, timeout=None, proxy=None, ssl=None):
        return _FakeRedirectResp(self._status, self._final_url, self._history)
...
@pytest.mark.asyncio
async def test_resolve_short_url_returns_last_real_douyin_url_before_404():
    client = DouyinAPIClient({"msToken": "t"})
    client._session = _FakeSession(
        404,
        "https://www.douyin.com/404",
        history=[
            _FakeHistoryItem("https://www.iesdouyin.com/share/video/7639590279997132072/"),
            _FakeHistoryItem("https://www.douyin.com/video/7639590279997132072?previous_page=web_code_link"),
        ],
    )
    resolved = await client.resolve_short_url("https://v.douyin.com/3yMwHgwkqkM/")
    assert resolved == "https://www.douyin.com/video/7639590279997132072?previous_page=web_code_link"
    await client.close()
...
def test_parse_live_room_info_from_page_extracts_inline_player_stream():
    stream_data = {
        "data": {
            "origin": {
                "main": {
                    "flv": "https://pull-flv.example/live_origin.flv",
                    "hls": "https://pull-hls.example/live_origin.m3u8",
                }
            },
            "hd": {
                "main": {
                    "flv": "https://pull-flv.example/live_hd.flv",
                    "hls": "https://pull-hls.example/live_hd.m3u8",
                }
            },
        }
    }
    ...
    html = (
        ...
        + '" data-anchor-info="{&quot;nickname&quot;:&quot;主播&quot;,&quot;id_str&quot;:&quot;42&quot;}" '
        + 'data-room-info="{&quot;roomId&quot;:&quot;123&quot;}"></script>'
    )

    parsed = DouyinAPIClient._parse_live_room_info_from_page(html, "80017709309")

    assert parsed["data"]["user"]["nickname"] == "主播"
    assert parsed["data"]["room"]["room_id_str"] == "123"
    assert parsed["data"]["room"]["web_rid"] == "80017709309"
    assert parsed["data"]["room"]["stream_url"]["flv_pull_url"]["ORIGIN"].endswith(
        "live_origin.flv"
    )
    assert parsed["data"]["room"]["stream_url"]["flv_pull_url"]["HD1"].endswith("live_hd.flv")
