...
class FileManager:
    ...
    async def download_file(
        ...
        *,
        ...
        async def fetch_to_file(*, ssl_verify: bool) -> Union[bool, Path]:
        ...
        try:
            try:
                return await fetch_to_file(ssl_verify=True)
            except aiohttp.ClientConnectorCertificateError as exc:
                logger.warning(
                    "Certificate verification failed while downloading %s, retry without SSL verification: %s",
                    final_path.name,
                    exc,
                )
                return await fetch_to_file(ssl_verify=False)
